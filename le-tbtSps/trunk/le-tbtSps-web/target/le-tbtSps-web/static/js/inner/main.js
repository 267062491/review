var MyPopWindow = {
	...
	SubmitCompletedCallback2 : function(data,url){
		...
		if($.trim(alertMsg).length > 0 ){dialog.alert(alertMsg);}
		var result = data.code== "200"; 	
		if(result && $.trim(alertMsg).length > 0 ){window.location.href = url;}
	},
    <!--提交成功后是否自动退出-->
	SubmitCompletedCallback3 : function(data,url){
		var alertMsg = data.message;
		if($.trim(alertMsg).length > 0 ){dialog.alert(alertMsg);}
		var result = data.code== "200";
		if(result && $.trim(alertMsg).length > 0 ){
            if ($("#submitExist").is(':checked')){
	...
	ExecutePost : function(msg, url, data) {
		dialog.confirm(msg,function(id){
	...
	ExecuteEnable : function(mes ,url, data) {
		MyPopWindow.ExecutePost(mes, url, data);
	},
	/**
     * 删除
     * @param jqGrideTable  table 的 id
     * @param columnNo table的第几列 从 1 开始
     * @param url 请求路径
     * @param stockReportGrid jqgird 实例
     * @param par   后台接受的参数名称
     * @constructor
     */
	ExecuteDeleteAjax : function(jqGrideTable,columnNo,url,stockReportGrid , par){
        var selectRowIds,selectRowWarehouses=[],selectRowId;
        selectRowIds = jQuery("#"+jqGrideTable).jqGrid('getGridParam', 'selarrrow');
        if(selectRowIds.length!=1){
            dialog.alert("请选择一条数据");
            return ;
        }else{
            var msg = "确定删除么？" ;debugger ;
            dialog.confirm(msg,function(id){
                selectRowId = $("#"+selectRowIds[0]+" td:eq("+columnNo+")").text()  ;
                var pars = {} ;
                pars[par]=selectRowId ;
                jQuery.post(url,
                    pars,
                    function(data) {
                        if(data!=null && data.code==200 ){
                            stockReportGrid.trigger('reloadGrid');
                        } else {
                            dialog.alert(data.message);
                        }
                    }
